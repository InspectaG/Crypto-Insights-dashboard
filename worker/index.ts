...
import { unauthorized } from "./access";
...
const worker = {
  async fetch(request: Request, env: WorkerEnv, ctx: WorkerExecutionContext): Promise<Response> {
    ...
    if (url.hostname === "crypto.gatchek.com" && unauthorized(request)) {
  ...
  scheduled(_controller: unknown, env: WorkerEnv, ctx: WorkerExecutionContext) {
